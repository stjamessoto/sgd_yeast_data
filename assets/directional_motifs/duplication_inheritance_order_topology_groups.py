"""
duplication_inheritance_order_topology_groups.py -- numbers the 21
five-family (k=5) combinations from duplication_inheritance_order.py and
groups them by directed-graph topology: which combos end up with the same
number of directed edges (this method never produces mutual pairs -- see
below).

WHY THIS IS A SEPARATE FILE
This is one of three sibling topology-group scripts (the others cover
tf_regulates_tf_cascade.py and dominant_coregulation_flow.py). Each
...
arrow colors -- this file's orange legend matches
duplication_inheritance_order.py's own LEGEND_HANDLES exactly, including
the "PROXY ONLY" caveat (see that script's module docstring for why: pi_i
is a single scalar per family, not literal ohnolog ancestry).

WHY THIS IS SEPARATE FROM THE EXISTING GRID
duplication_inheritance_order.py already draws all 21 combos as a grid,
...
Every edge is directed by a single, fixed, per-family inheritance
probability pi_i ranking that never depends on which other families are
in the 5-subset, and a strict ranking can never produce a mutual pair (A
can't simultaneously have higher AND lower pi_i than B). So the only thing
that varies combo to combo is whether F7 (zero pi4 coverage, always
...
Run: python assets/directional_motifs/duplication_inheritance_order_topology_groups.py
"""
...
OUT_PNG = OUT_DIR / "duplication_inheritance_order_topology_groups.png"
...
fig.suptitle(
    "Duplication/inheritance order (πᵢ ranking, PROXY ONLY)\n"
